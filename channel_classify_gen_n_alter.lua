...
netG:add(SpatialFullConvolution(ngf,ngen*nc, 4, 4, 2, 2, 1, 1))
netG:add(nn.Tanh())
-- state size: (nc) x 64 x 64
netG:add(nn.Reshape(ngen,nc,64,64,true))
...
local fDx = function(x)
   ...
   local errD_real=0
   for i=1,ngen do
       input:copy(fakes[{{1,opt.batchSize},i}])
...
local fGx = function(x)
   ...
   errG=0
   local df_dgs=netG.output:clone()
   for i=1,ngen do
     local output = netD:forward(netG.output[{{1,opt.batchSize},i}]) -- netD:forward(input) was already executed in fDx, so save computation
     
     errG =errG+ criterion:forward(output, label)
     local df_do = criterion:backward(output, label)
     local df_dg = netD:updateGradInput(netG.output[{{1,opt.batchSize}, i}], df_do)
     df_dgs[{{1,opt.batchSize},i}]= df_dg
   end
...
for epoch = 1, opt.niter do 
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % 10 == 0 and opt.display then
          ...
          for i=1,ngen do
             disp.image(fakes[{{1,opt.batchSize},i}], {win=opt.display_id * 3 +i, title=opt.name})
          end
      end
