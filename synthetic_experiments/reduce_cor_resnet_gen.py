...
parser.add_argument('--lambda_cor', type=float, default=1.0, help='lambda for correlation of the outputs of various residual blocks default=0')
parser.add_argument('--beta1', type=float, default=0.5, help='beta1 for adam. default=0.5')
parser.add_argument('--cuda', action='store_true', help='enables cuda')
parser.add_argument('--ngpu', type=int, default=1, help='number of GPUs to use')
parser.add_argument('--netG', default='', help="path to netG (to continue training)")
parser.add_argument('--netD', default='', help="path to netD (to continue training)")
parser.add_argument('--outf', default='./reduce_cor_resnet_gen/', help='folder to output images and model checkpoints')
...
def compute_pairwise_cor(activations):
    pairwise_cor = 0
    for i in range(len(activations)):
        for j in xrange(i+1,len(activations)):
            act_i = activations[i].resize(opt.batchSize * opt.ngf)
            act_j = activations[j].resize(opt.batchSize * opt.ngf)
            pairwise_cor +=torch.abs( torch.dot(act_i,act_j) )
    return pairwise_cor
...
for epoch in range(opt.niter):
    ...
    for i,data in enumerate(dataloader,0):
        ...
        noisev = Variable(noise)
        # Magic with the intermediate residual blocks
        out = noisev
        activations=[]
        ...
        output = netD(fake)
        errG = criterion(output, labelv) + opt.lambda_cor*compute_pairwise_cor(activations)
        errG.backward()
