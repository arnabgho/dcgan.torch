...
class _netG(nn.Module):
    def __init__(self,ngpu):
        ...
        main_block+=[SpectralNorm(nn.Linear(opt.nz,opt.ngf) ) ]

        # Sets of residual blocks start

        for i in range(opt.ngres):
            main_block+= [ SpectralNorm( ResBlock(opt.ngf,opt.dropout) ) ] #[BATCHResBlock(opt.ngf,opt.dropout)]

        # Final layer to map to 1D

        main_block+=[ SpectralNorm( nn.Linear(opt.ngf,opt.out_dim) ) ]
...
class _netD(nn.Module):
    def __init__(self,ngpu):
        ...
        main_block+=[SpectralNorm(nn.Linear(opt.out_dim,opt.ngf)) ]
        main_block+=[nn.ReLU()]
        # Sets of residual blocks start
        ...
        main_block+=[SpectralNorm(nn.Linear(opt.ngf,1))]
