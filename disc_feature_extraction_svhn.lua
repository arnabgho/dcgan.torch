...
if opt.mode=="train" then
	features_train = torch.Tensor(num_batches_train*opt.batchSize, 512*4*4)
end
if opt.mode=="test" then
	features_test = torch.Tensor(num_batches_test*opt.batchSize, 512*4*4)
end
...
for i=0,num_batches_train-1 do
   ...
   features_train[ {{i*opt.batchSize+1 , (i+1)*opt.batchSize   }}  ]=D_features
end
...
for i=0,num_batches_test-1 do
   ...
   D:forward( image_batch )
   local D_features=D.modules[11].output:reshape(opt.batchSize,512*4*4):float()
   features_test[ {{i*opt.batchSize+1 , (i+1)*opt.batchSize   }}  ]=D_features 
end
