...
opt = {
   ...
   name = 'experiment-conceding-message1',
   noise = 'normal',       -- uniform / normal
   eps=0.001,	           -- message visualization eps
}
...
if opt.gpu > 0 then
   ...
   input = input:cuda();  noise = noise:cuda();  label = label:cuda() ; message_G1=message_G1:cuda() ; message_G2 = message_G2:cuda() ; zero_batch=zero_batch:cuda() ; D_G1_out=D_G1_out:cuda() ; D_G2_out=D_G2_out:cuda() ; prev_fake1 = prev_fake1:cuda() ; prev_fake2=prev_fake2:cuda()
...
if opt.noise == 'uniform' then
    ...
    m_noise:uniform(-eps,eps)
elseif opt.noise == 'normal' then
    noise_vis:normal(0, 1)
    m_noise:normal(0,eps)
end
...
local fGx = function(x)
   ...
   local df_input2= G.netG2:backward(torch.cat(noise,message_G1,2),df_dg)
   ...
   local df_input1=G.netG1:backward(torch.cat(noise,message_G2,2),df_dg)
...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % 10 == 0 and opt.display then
          ...
          disp.image(m_vis1, {win=opt.display_id+3 , title=opt.name .. "_message_vis"})
          disp.image(m_vis2, {win=opt.display_id+4 , title=opt.name .. "_message_vis"})
      end
