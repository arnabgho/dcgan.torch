...
parser.add_argument('--outf', default='./vaegan_enhancer', help='folder to output images and model checkpoints')
...
class _netD(nn.Module):
    def __init__(self, imageSize, ngpu):
        ...
        self.main.add_module('output-conv', nn.Conv2d(ndf * 2**(n-3), 3, 4, 1, 0, bias=False))
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        label.data.fill_(enhanced_label)
        enhanced=netE(gen.detach())
        output=netD(enhanced.detach())
        errD_enhanced = criterion( output,label )
