...
def test(epoch):
    ...
    for i, data in enumerate(test_loader,0):
        ...
        recon_batch = netG.decoder(sampled)

        MSEerr = MSECriterion(recon_batch,input)
        L1_err = L1Criterion(recon_batch,input)
        test_loss += MSEerr
        l1_loss += L1_err
        if i == 0:
          n =batch_size#data.size(0) #min(data.size(0), 8)
          comparison = torch.cat([input[:n],
                                  recon_batch.view(opt.batchSize, 3, opt.imageSize, opt.imageSize)[:n]])
          #comparison = torch.cat([data,
          #                        recon_batch.view(batch_size, 3, opt.imageSize, opt.imageSize)])
          save_image(comparison.data.cpu(),
                     opt.outf + '/reconstruction_' + str(epoch) + '.png', nrow=n)

    test_loss /= len(test_loader.dataset)
    l1_loss /= len(test_loader.dataset)
    #print('====> Test set MSE loss: {:.4f}'.format(test_loss))
    #print('====> Test set L1 loss: {:.4f}'.format(test_loss))
    print('MSE Loss')
    print('===================================')
    print(test_loss)
    print('===================================')
