...
noise1 = torch.Tensor(opt.batchSize, opt.nz, opt.imsize, opt.imsize)
noise2 = torch.Tensor(opt.batchSize, opt.nz, opt.imsize, opt.imsize)
if opt.gpu > 0 then
    require 'cunn'
    require 'cudnn'
    --`net:cuda()
...
elseif opt.noisetype == 'normal' then
    noise1:normal(0, 1)
    noise2:normal(0,1)
    noiseL = torch.FloatTensor(opt.nz):normal(0,1)
    noiseR = torch.FloatTensor(opt.nz):normal(0,1)
end
...
local images_G1 = G.netG1:forward(noise1)
...
local images_G2 = G.netG2:forward(noise2,message_G1,2)
