...
opt = {
    ...
    ngen = 3,               -- the number of generators generating images                
...
for epoch = 1, opt.niter do
    paths.mkdir('checkpoints_heads_classify_gen_n_alter')
    epoch_tm:reset()
    local counter = 0
	...
	local fake=G['netG'..i]:forward(noise_vis)
        ...
        for i=1,opt.batchSize do
            if image_out==nil then 
                ...
                image_out=torch.cat(image_out,fake[i],2)
            end
        end

    	image.save(paths.concat('checkpoints_heads_classify_gen_n_alter/', 'gen_'..i .. '_epoch_'..epoch..'.jpg'),image_out)
    end	
   ...
   if epoch % opt.save_freq==0 then	
       ...
       torch.save('checkpoints_heads_classify_gen_n_alter/' .. opt.name .. '_' .. epoch .. '_net_G.t7', {G=G} )
       torch.save('checkpoints_heads_classify_gen_n_alter/' .. opt.name .. '_' .. epoch .. '_net_D.t7', netD )
    end	
